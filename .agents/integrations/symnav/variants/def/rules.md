...
Using `symnav def` is required for this task. Reach for it ahead of grep, find, and plain file reads for any question about the TypeScript code — what is in a file, where a symbol is defined, who references it, or how calls flow. Prefer it over the usual tools; do not decide it is not worth running.
