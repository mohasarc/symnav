...
describe("DaemonProcessTerminationObserver", () => {
  it.each([
    ["uncaught exception", "uncaught-exception", (observer: DaemonProcessTerminationObserver) => observer.uncaughtException(new Error("/secret/path"), "uncaughtException")],
    ["unhandled rejection", "unhandled-rejection", (observer: DaemonProcessTerminationObserver) => observer.unhandledRejection(new TypeError("token=secret"))],
    ["signal", "signal", (observer: DaemonProcessTerminationObserver) => observer.signal("SIGTERM")],
  ] as const)("records one closed redacted %s classification before cleanup", async (_label, reason, terminate) => {
