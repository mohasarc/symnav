...
describe("DaemonScaleBenchmarkHarness", () => {
  it.skipIf(process.platform === "win32")(
    "runs one cold baseline and nine warm fixed-suite repetitions with aggregate evidence",
    async () => {
      ...
      expect(artifact.statusMaximumMs).toBeGreaterThan(0);
      expect(artifact.processRssPeakBytes).toBeGreaterThan(0);
      expect(artifact.processRssPeakBytes).toBeLessThan(
        artifact.resourcePolicy.hardProcessRssBytes,
      );
