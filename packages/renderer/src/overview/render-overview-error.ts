...
export function renderOverviewError(err: unknown): string | undefined {
  if (err instanceof AmbiguousOverviewError) {
    return renderCandidateError(ambiguityStatementFor(err), err.candidates);
  }
  if (err instanceof OverviewTargetNotFoundError) {
    return `Cannot answer: no overview target matching ${formatRequest(err.request)}.\n`;
  }
  return undefined;
}

function ambiguityStatementFor(err: AmbiguousOverviewError): string {
